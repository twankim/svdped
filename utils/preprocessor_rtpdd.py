...
def random_horizontal_flip(image,
  ...
  Args:
    ...
    lidar_front: rank 3 float32 tensor with shape [height, width, channels].
  ...
  Returns:
    image: image which is the same shape as input image.
    lidar_front: lidar front image which is the same shape as input.
  ...
  with tf.name_scope('RandomHorizontalFlip', values=[image,lidar_front,boxes]):
    result = []
    ...
    # flip front lidar image
    lidar_front = tf.cond(do_a_flip_random, 
        lambda: _flip_image(lidar_front), lambda: lidar_front)
    result.append(lidar_front)
...
def _strict_random_crop_image_lidar(
                              image,
                              lidar_front,
                              boxes,
                              labels,
  ...
  Args:
    image: rank 3 float32 tensor containing 1 image -> [height, width, channels]
           with pixel values varying between [0, 1].
    lidar_front: rank 3 float32 tensor contains 1 lidar front image 
  ...
  Returns:
    image: image which is the same rank as input image.
    lidar_front: lidar front image which is the same rank as input.
    boxes: boxes which is the same rank as input boxes.
           Boxes are in normalized form.
    labels: new labels.
    ...
    keypoints: rank 3 float32 tensor with shape
               [num_instances, num_keypoints, 2]
  """
  with tf.name_scope('RandomCropImageLIDAR', values=[image,lidar_front,boxes]):
    image_shape = tf.shape(image)
    ...
    r_imlidar = image_shape[1]/tf.shape(lidar_front)[1]
    new_lidar_front = tf.slice(lidar_front, im_box_begin/r_imlidar, im_box_size/r_imlidar)
    new_lidar_front.set_shape([None, None, lidar_front.get_shape()[2]])
    ...
    result = [new_image, new_lidar_front, new_boxes, new_labels]
...
def random_crop_image_lidar(
                      ...
                      random_coef=0.0,
                      seed=None):
  """Randomly crops the image and lidar front image
  ...
  Returns:
    image: Image shape will be [new_height, new_width, channels].
    lidar_front: Lidar shape will be [new_height, new_width, channels].
    boxes: boxes which is the same rank as input boxes. Boxes are in normalized
           form.
    labels: new labels.
  ...
  def strict_random_crop_image_lidar_fn():
    return _strict_random_crop_image_lidar(
        image,
        lidar_front,
        boxes,
        labels,
  ...
  # avoids tf.cond to make faster RCNN training on borg. See b/140057645.
  if random_coef < sys.float_info.min:
    result = strict_random_crop_image_lidar_fn()
  else:
    do_a_crop_random = tf.random_uniform([], seed=seed)
    do_a_crop_random = tf.greater(do_a_crop_random, random_coef)

    outputs = [image, lidar_front, boxes, labels]
    ...
    result = tf.cond(do_a_crop_random, strict_random_crop_image_lidar_fn,
                     lambda: tuple(outputs))
  return result
...
def ssd_random_crop(image,
  ...
  Args:
    image: rank 3 float32 tensor contains 1 image -> [height, width, channels]
           with pixel values varying between [0, 1].
    image: rank 3 float32 tensor contains 1 lidar front image 
           -> [height, width, channels]
           with pixel values varying between [0, 1].
    boxes: rank 2 float32 tensor containing the bounding boxes -> [N, 4].
           Boxes are in normalized form meaning their coordinates vary
  ...
  def random_crop_selector(selected_result, index):
    ...
    """
    i = 4
    image, lidar_front, boxes, labels = selected_result[:i]
    ...
    return random_crop_image_lidar(
        image=image,
  ...
  result = _apply_with_random_selector_tuples(
      tuple(
          t for t in (image, lidar_front, boxes,
                      labels, label_scores, masks, keypoints)
          if t is not None),
...
def get_default_func_arg_map(include_label_scores=False,
  ...
  prep_func_arg_map = {
      ...
      random_horizontal_flip: (
          ...
          fields.InputDataFields.lidar_front,
...
def preprocess(tensor_dict, preprocess_options, func_arg_map=None):
  ...
  # changes the lidar front images to image (rank 4 to rank 3)
  if fields.InputDataFields.lidar_front in tensor_dict:
    lidars_front = tensor_dict[fields.InputDataFields.lidar_front]
    if len(lidars_front.get_shape()) != 4:
      raise ValueError('lidar front images in tensor_dict should be rank 4')
    lidar_front = tf.squeeze(lidars_front, squeeze_dims=[0])
    tensor_dict[fields.InputDataFields.lidar_front] = lidar_front
  ...
  # changes the lidar front image to image (rank 3 to rank 4)
  if fields.InputDataFields.lidar_front in tensor_dict:
    lidar_front = tensor_dict[fields.InputDataFields.lidar_front]
    lidars_front = tf.expand_dims(lidar_front, 0)
    tensor_dict[fields.InputDataFields.lidar_front] = lidars_front
