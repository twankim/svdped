...
from utils import box_predictor_rtpdd as bpredictor
from utils import target_assigner_rtpdd as target_assigner
...
class SSDMetaArch(model.DetectionModel):
  ...
  def __init__(self,
    ...
    self._distance_loss = distance_loss
    self._classification_loss_weight = classification_loss_weight
    self._localization_loss_weight = localization_loss_weight
    self._distance_loss_weight = distance_loss_weight
  ...
  def predict(self, preprocessed_inputs, preprocessed_lidars):
    ...
    Args:
      ...
      preprocessed_lidarss: a [batch, height, width, channels] lidar_front tensor.
    ...
    with tf.variable_scope(None, self._extract_features_scope,
                           [preprocessed_inputs,preprocessed_lidars]):
    ...
    (box_encodings, class_predictions_with_background, dist_predictions
  ...
  def _add_box_predictions_to_feature_maps(self, feature_maps):
    ...
    Returns:
      ...
      class_predictions_with_background: 3-D float tensor of shape
          ...
          (logits) for each of the anchors.  Note that this tensor *includes*
          background class predictions (at class index 0).
      dist_predictions: 3-D float tensor of shape [batch_size, num_anchors, 1]
          containing predicted distances.
    ...
    for idx, (feature_map, num_anchors_per_location
      ...
      dist_predictions = box_predictions[bpredictor.DIST_PREDICTIONS]
      ...
      dist_predictions_list.append(dist_predictions)
    ...
    with tf.control_dependencies([anchors_assert]):
      ...
      dist_predictions = tf.concat(
          dist_predictions_list, 1)
    return box_encodings, class_predictions_with_background, dist_predictions
  ...
  def loss(self, prediction_dict, scope=None):
    ...
    Args:
      prediction_dict: a dictionary holding prediction tensors with
        ...
        2) class_predictions_with_background: 3-D float tensor of shape
          ...
          (logits) for each of the anchors. Note that this tensor *includes*
          background class predictions.
        3) dist_predictions: 3-D float tensor of shape
          [batch_size, num_anchors, 1] containing distance predictions
    ...
    with tf.name_scope(scope, 'Loss', prediction_dict.values()):
      ...
      (batch_cls_targets, batch_cls_weights, batch_reg_targets,
       batch_reg_weights, batch_dist_targets, batch_dist_weights,
       match_list) = self._assign_targets(
           self.groundtruth_lists(fields.BoxListFields.boxes),
           self.groundtruth_lists(fields.BoxListFields.classes),
           self.groundtruth_lists(fields.BoxListFields.dists),
           keypoints)
      ...
      dist_losses = self._distance_loss(
          prediction_dict['dist_predictions'],
          batch_dist_targets,
          weights=batch_dist_weights)

      #TODO: hardexampleminer
      if self._hard_example_miner:
        (localization_loss, classification_loss,
         distance_loss) = self._apply_hard_mining(
                location_losses, cls_losses, dist_losses,
                prediction_dict, match_list)
        if self._add_summaries:
        ...
        distance_loss = tf.reduce_sum(dist_losses)
      ...
      with tf.name_scope('distance_loss'):
        distance_loss = ((self._distance_loss_weight / normalizer) *
                               distance_loss)
  ...
  def _assign_targets(self, groundtruth_boxes_list, groundtruth_classes_list,
                      groundtruth_dists_list, groundtruth_keypoints_list=None):
  ...
  def _apply_hard_mining(self, location_losses, cls_losses, dist_losses,
                         prediction_dict, match_list):
    ...
    Args:
      ...
      dist_losses: Float tensor of shape [batch_size, num_anchors]
        representing anchorwise distance losses.
      prediction_dict: p a dictionary holding prediction tensors with
        ...
        2) class_predictions_with_background: 3-D float tensor of shape
          ...
          (logits) for each of the anchors.  Note that this tensor *includes*
          background class predictions.
        2) dist_predictions: 3-D float tensor of shape
          [batch_size, num_anchors, 1] containing distance predictions 
    ...
    dist_prediction_list = tf.unstack(prediction_dict['dist_predictions'])
  ...
  def provide_groundtruth(self,
    ...
    self._groundtruth_lists[fields.BoxListFields.dists] = groundtruth_dists_list
