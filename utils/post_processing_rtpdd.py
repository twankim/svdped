...
def multiclass_non_max_suppression(boxes,
  ...
  with tf.name_scope(scope, 'MultiClassNonMaxSuppression'):
    ...
    per_class_dists_list = tf.unstack(dists, axis=1)
    ...
    for class_idx, boxes_idx in zip(range(num_classes), boxes_ids):
      ...
      boxlist_and_class_scores.add_field(fields.BoxListFields.scores,
                                         class_scores)
      per_class_dists = per_class_dists_list[boxes_idx]
      boxlist_and_class_scores.add_field(fields.BoxListFields.dists,
                                         per_class_dists)
      if masks is not None:
      ...
      nms_result.add_field(
          fields.BoxListFields.classes, (tf.zeros_like(
              nms_result.get_field(fields.BoxListFields.scores)) + class_idx+1))
...
def batch_multiclass_non_max_suppression(boxes,
  ...
  with tf.name_scope(scope, 'BatchMultiClassNonMaxSuppression'):
    ...
    def _single_image_nms_fn(args):
      ...
      per_image_dists = tf.reshape(
          tf.slice(per_image_dists, [0, 0],
                   tf.stack([per_image_num_valid_boxes, -1])),
          [-1, 1])
      ...
      nmsed_dists = padded_boxlist.get_field(fields.BoxListFields.dists)
      nmsed_additional_fields = [
          padded_boxlist.get_field(key) for key in per_image_additional_fields
      ]
      return ([nmsed_boxes, nmsed_scores, nmsed_classes, nmsed_masks, nmsed_dists]
    ...
    batch_outputs = tf.map_fn(
        ...
        elems=([boxes, scores, masks, dists] + list(additional_fields.values()) +
               [num_valid_boxes]),
    ...
    batch_nmsed_dists = batch_outputs[4]
    batch_nmsed_additional_fields = {
        key: value
        for key, value in zip(additional_fields, batch_outputs[5:-1])
    }
    ...
    return (batch_nmsed_boxes, batch_nmsed_scores, batch_nmsed_classes,
            batch_nmsed_masks, batch_nmsed_dists, 
