...
class WeightedL2DistanceLoss(Loss):
  """L2 distance loss function with anchorwise output support.
  ...
  def _compute_loss(self, prediction_tensor, target_tensor, weights):
    ...
    Args:
      prediction_tensor: A float tensor of shape [batch_size, num_anchors, 1] 
        representing the predicted radial distances of objects.
      target_tensor: A float tensor of shape [batch_size, num_anchors, 1]
    ...
    """
    # Calculate loss only when the true distance > 0 (Exist)
    indicator_nonzero = tf.cast(target_tensor>0,tf.float32)
    weighted_diff = (prediction_tensor - target_tensor) * tf.expand_dims(
        weights, 2) * indicator_nonzero
...
class HardExampleMiner(object):
  ...
  def __call__(self,
    ...
    dist_losses = tf.unstack(dist_losses)
    ...
    for ind, detection_boxlist in enumerate(decoded_boxlist_list):
      ...
      mined_dist_losses.append(
          tf.reduce_sum(tf.gather(dist_losses[ind], selected_indices)))
    location_loss = tf.reduce_sum(tf.stack(mined_location_losses))
    cls_loss = tf.reduce_sum(tf.stack(mined_cls_losses))
    dist_loss = tf.reduce_sum(tf.stack(mined_dist_losses))
    ...
    return (location_loss, cls_loss, dist_loss)
