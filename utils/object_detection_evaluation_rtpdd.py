...
from utils import per_image_evaluation_rtpdd as per_image_evaluation
from utils.utils_distance import compute_rmse_dist
...
class ObjectDetectionEvaluator(DetectionEvaluator):
  ...
  def add_single_ground_truth_image_info(self, image_id, groundtruth_dict):
    ...
    Args:
      ...
      groundtruth_dict: A dictionary containing -
        fields.InputDataFields.groundtruth_boxes: float32 numpy array
          of shape [num_boxes, 4] containing `num_boxes` groundtruth boxes of
          the format [ymin, xmin, ymax, xmax] in absolute image coordinates.
        fields.InputDataFields.groundtruth_classes: integer numpy array
          of shape [num_boxes] containing 1-indexed groundtruth classes for the
          boxes.
        fields.InputDataFields.groundtruth_difficult: Optional length
    ...
    groundtruth_dists = groundtruth_dict[
    ...
    if (fields.InputDataFields.groundtruth_difficult in
        groundtruth_dict.keys() and
        (groundtruth_dict[fields.InputDataFields.groundtruth_difficult]
    ...
    self._evaluation.add_single_ground_truth_image_info(
        image_id,
        groundtruth_dict[fields.InputDataFields.groundtruth_boxes],
        groundtruth_classes,
        groundtruth_dists,
  ...
  def add_single_detected_image_info(self, image_id, detections_dict):
    ...
    Args:
      ...
      detections_dict: A dictionary containing -
        fields.DetectionResultFields.detection_boxes: float32 numpy
          array of shape [num_boxes, 4] containing `num_boxes` detection boxes
          of the format [ymin, xmin, ymax, xmax] in absolute image coordinates.
        fields.DetectionResultFields.detection_scores: float32 numpy
          array of shape [num_boxes] containing detection scores for the boxes.
        fields.DetectionResultFields.detection_classes: integer numpy
    ...
    self._evaluation.add_single_detected_image_info(
        ...
        detections_dict[fields.DetectionResultFields.detection_scores],
        detection_classes,
        detections_dict[fields.DetectionResultFields.detection_dists])
  ...
  def evaluate(self):
    ...
    (per_class_ap, mean_ap, _, _, per_class_corloc, mean_corloc, 
     per_class_rmse, mean_rmse) = (
        self._evaluation.evaluate())
    print(per_class_rmse, mean_rmse)
    pascal_metrics = {
        self._metric_prefix +
        'Precision/mAP@{}IOU'.format(self._matching_iou_threshold):
            mean_ap
    }
    display_name = self._metric_prefix +'Distance/meanRMSE@{}IOU'.format(
            self._matching_iou_threshold)
    pascal_metrics[display_name] = mean_rmse
    ...
    for idx in range(per_class_ap.size):
      if idx + self._label_id_offset in category_index:
        ...
        display_name = (
            self._metric_prefix + 'PerformanceByCategory/RMSE@{}IOU/{}'.format(
                self._matching_iou_threshold,
                category_index[idx + self._label_id_offset]['name']))
        pascal_metrics[display_name] = per_class_rmse[idx]
...
class OpenImagesDetectionEvaluator(ObjectDetectionEvaluator):
  ...
  def add_single_ground_truth_image_info(self, image_id, groundtruth_dict):
    ...
    Args:
      ...
      groundtruth_dict: A dictionary containing -
        ...
        fields.InputDataFields.groundtruth_classes: integer numpy array
          of shape [num_boxes] containing 1-indexed groundtruth classes for the
          boxes.
        fields.InputDataFields.groundtruth_group_of: Optional length
    ...
    # (unless there are no annotations for the groundtruth on this image)
    # use values from the dictionary or insert None otherwise.
    if (fields.InputDataFields.groundtruth_group_of in
        groundtruth_dict.keys() and
        (groundtruth_dict[fields.InputDataFields.groundtruth_group_of]
         .size or not groundtruth_classes.size)):
...
class ObjectDetectionEvaluation(object):
  ...
  def __init__(self,
    ...
    self.abs_diffs_per_class = [[] for _ in range(self.num_class)]
    self.is_dists_per_class = [[] for _ in range(self.num_class)]
    self.num_images_correctly_detected_per_class = np.zeros(self.num_class)
    self.average_precision_per_class = np.empty(self.num_class, dtype=float)
    self.rmse_dist_per_class = np.empty(self.num_class, dtype=float)
  ...
  def add_single_ground_truth_image_info(self,
    ...
    self.groundtruth_dists[image_key] = groundtruth_dists
  ...
  def add_single_detected_image_info(self, image_key, detected_boxes,
    ...
    if (len(detected_boxes) != len(detected_scores) or
        len(detected_boxes) != len(detected_class_labels) or
        len(detected_boxes) != len(detected_dists)):
      raise ValueError('detected_boxes, detected_scores and '
                       'detected_class_labels and'
                       'detected_dists should all have same lengths. Got'
                       '[%d, %d, %d, %d]' % len(detected_boxes),
    ...
    if image_key in self.groundtruth_boxes:
      ...
      groundtruth_dists = self.groundtruth_dists[image_key]
      ...
      groundtruth_dists = np.array([], dtype=float)
      groundtruth_is_difficult_list = np.array([], dtype=bool)
      groundtruth_is_group_of_list = np.array([], dtype=bool)
    scores,tp_fp_labels,is_class_correctly_detected_in_image,abs_diffs,is_dists = (
        self.per_image_eval.compute_object_detection_metrics(
            detected_boxes, detected_scores, detected_class_labels,
            detected_dists,
            groundtruth_boxes, groundtruth_class_labels, groundtruth_dists,
    ...
    for i in range(self.num_class):
      if scores[i].shape[0] > 0:
        ...
        self.abs_diffs_per_class[i].append(abs_diffs[i])
        self.is_dists_per_class[i].append(is_dists[i])
  ...
  def evaluate(self):
    ...
    for class_index in range(self.num_class):
      ...
      if not self.scores_per_class[class_index]:
        ...
        abs_diffs = np.concatenate(self.abs_diffs_per_class[class_index])
        is_dists = np.concatenate(self.is_dists_per_class[class_index])
      if self.use_weighted_mean_ap:
      ...
      self.rmse_dist_per_class[class_index] = compute_rmse_dist(abs_diffs,is_dists)
    ...
    mean_rmse = np.nanmean(self.rmse_dist_per_class)
    return ObjectDetectionEvalMetrics(
        ...
        self.rmse_dist_per_class, mean_rmse)
