...
class PerImageEvaluation(object):
  ...
  def compute_object_detection_metrics(
    ...
    """
    detected_boxes, detected_scores, detected_class_labels, detected_dists = (
        self._remove_invalid_boxes(detected_boxes, detected_scores,
                                   detected_class_labels, detected_dists))
    scores, tp_fp_labels, abs_diffs, is_dists = self._compute_tp_fp_abs(
        detected_boxes, detected_scores, detected_class_labels,
        detected_dists, groundtruth_boxes, groundtruth_class_labels,
        groundtruth_dists,
        groundtruth_is_difficult_lists, groundtruth_is_group_of_list)
  ...
  def _compute_tp_fp_abs(self, detected_boxes, detected_scores,
                     detected_class_labels, detected_dists, groundtruth_boxes,
                     groundtruth_class_labels, groundtruth_dists,
    ...
    for i in range(self.num_groundtruth_classes):
      ...
      gt_dists_at_ith_class = groundtruth_dists[groundtruth_class_labels == i]
      ...
      detected_scores_at_ith_class = detected_scores[detected_class_labels == i]
      detected_dists_at_ith_class = detected_dists[detected_class_labels == i]
      (scores, tp_fp_labels, abs_diffs, is_dists) = (
          self._compute_tp_fp_abs_for_single_class(
              detected_boxes_at_ith_class, detected_scores_at_ith_class,
              detected_dists_at_ith_class, gt_boxes_at_ith_class,
              gt_dists_at_ith_class, groundtruth_is_difficult_list_at_ith_class,
              groundtruth_is_group_of_list_at_ith_class))
      result_scores.append(scores)
      result_tp_fp_labels.append(tp_fp_labels)
      result_abs_diffs.append(abs_diffs)
      result_is_dists.append(is_dists)
    return result_scores, result_tp_fp_labels, result_abs_diffs, result_is_dists
  ...
  def _remove_invalid_boxes(self, detected_boxes, detected_scores,
    ...
    return (detected_boxes[valid_indices, :], detected_scores[valid_indices],
            detected_class_labels[valid_indices], detected_dists[valid_indices])

  def _compute_tp_fp_abs_for_single_class(
      self, detected_boxes, detected_scores, detected_dists, groundtruth_boxes,
      groundtruth_dists, groundtruth_is_difficult_list,
      groundtruth_is_group_of_list):
    ...
    detected_boxlist.add_field('dists', detected_dists)
    ...
    dists = detected_boxlist.get_field('dists')

    if groundtruth_boxes.size == 0:
      return (scores, np.zeros(detected_boxlist.num_boxes(), dtype=bool),
              np.zeros(detected_boxlist.num_boxes(),dtype=float),
              np.zeros(detected_boxlist.num_boxes(), dtype=bool))

    tp_fp_labels = np.zeros(detected_boxlist.num_boxes(), dtype=bool)
    abs_diffs = np.zeros(detected_boxlist.num_boxes(), dtype=float)
    is_dists = np.zeros(detected_boxlist.num_boxes(), dtype=bool)
    ...
    gt_non_group_of_dists = groundtruth_dists[~groundtruth_is_group_of_list]
        ...
        if iou[i, gt_id] >= self.matching_iou_threshold:
          if not groundtruth_nongroup_of_is_difficult_list[gt_id]:
            if not is_gt_box_detected[gt_id]:
              ...
              if gt_non_group_of_dists[gt_id] > 0:
                is_dists[i] = True
                abs_diffs[i] = abs(dists[i]-gt_non_group_of_dists[gt_id])
          else:
    ...
    return (
        ...
        abs_diffs[~is_matched_to_difficult_box & ~is_matched_to_group_of_box],
        is_dists[~is_matched_to_difficult_box & ~is_matched_to_group_of_box])
