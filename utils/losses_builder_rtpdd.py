...
from utils import losses_rtpdd as losses


def build(loss_config,dist_config=None):
  ...
  if dist_config is not None:
    distance_loss = _build_distance_loss(
      dist_config)
    distance_weight = dist_config['distance_weight']
  else:
    distance_loss = None
  ...
  if loss_config.HasField('hard_example_miner'):
    ...
    hard_example_miner = build_hard_example_miner(
        ...
        localization_weight,
        distance_weight)
  return (classification_loss, localization_loss, distance_loss,
...
def build_hard_example_miner(config,
  ...
  hard_example_miner = losses.HardExampleMiner(
      ...
      dist_loss_weight=distance_weight,
...
def _build_distance_loss(dist_config):
  """Builds a distance loss based on the loss config.

  Args:
    dist_config: A dictionary of config for distance loss 

  Returns:
    Loss based on the config.

  Raises:
    ValueError: On invalid dist_config.
  """
  loss_type = dist_config['distance_loss']
  if loss_type == 'weighted_l2':
    return losses.WeightedL2DistanceLoss(
        anchorwise_output=dist_config['anchorwise_output'])

  raise ValueError('Empty distance loss config.')
