...
class TargetAssigner(object):
  ...
  def __init__(self, similarity_calc, matcher, box_coder,
    ...
    self._unmatched_dist_target = tf.constant([0], tf.float32)
  ...
  def assign(self, anchors, groundtruth_boxes, groundtruth_labels=None,
    ...
    with tf.control_dependencies(
      ...
      cls_targets = self._create_classification_targets(groundtruth_labels,
                                                        match)
      dist_targets = self._create_distance_targets(groundtruth_dists,
                                                   match)
      ...
      dist_weights = self._create_distance_weights(match)

      num_anchors = anchors.num_boxes_static()
      if num_anchors is not None:
        reg_targets = self._reset_target_shape(reg_targets, num_anchors)
        cls_targets = self._reset_target_shape(cls_targets, num_anchors)
        dist_targets = self._reset_target_shape(dist_targets, num_anchors)
        reg_weights = self._reset_target_shape(reg_weights, num_anchors)
        cls_weights = self._reset_target_shape(cls_weights, num_anchors)
        dist_weights = self._reset_target_shape(dist_weights, num_anchors)

    return (cls_targets, cls_weights, reg_targets, reg_weights,
            dist_targets, dist_weights, match)
  ...
  def _create_distance_targets(self, groundtruth_dists, match):
    matched_anchor_indices = match.matched_column_indices()
    unmatched_ignored_anchor_indices = (match.
                                        unmatched_or_ignored_column_indices())
    matched_gt_indices = match.matched_row_indices()
    matched_dist_targets = tf.gather(groundtruth_dists, matched_gt_indices)
    
    unmatched_ignored_dist_targets = tf.tile(
        tf.expand_dims(self._unmatched_dist_target, 0),
        tf.stack([tf.size(unmatched_ignored_anchor_indices), 1]))
    
    dist_targets = tf.dynamic_stitch(
        [matched_anchor_indices, unmatched_ignored_anchor_indices],
        [matched_dist_targets, unmatched_ignored_dist_targets])
    return dist_targets
  ...
  def _create_distance_weights(self, match):
    dist_weights = tf.cast(match.matched_column_indicator(), tf.float32)
    return dist_weights
...
def batch_assign_targets(target_assigner,
  ...
  dist_weights_list = []
  match_list = []
  for anchors, gt_boxes, gt_class_targets, gt_dists in zip(
      anchors_batch, gt_box_batch, gt_class_targets_batch, gt_dist_batch):
    (cls_targets, cls_weights, reg_targets,
     reg_weights, dist_targets, dist_weights, match) = target_assigner.assign(
         anchors, gt_boxes, gt_class_targets, gt_dists)
    cls_targets_list.append(cls_targets)
    cls_weights_list.append(cls_weights)
    reg_targets_list.append(reg_targets)
    reg_weights_list.append(reg_weights)
    dist_targets_list.append(dist_targets)
    dist_weights_list.append(dist_weights)
    match_list.append(match)
  batch_cls_targets = tf.stack(cls_targets_list)
  batch_cls_weights = tf.stack(cls_weights_list)
  batch_reg_targets = tf.stack(reg_targets_list)
  batch_reg_weights = tf.stack(reg_weights_list)
  batch_dist_targets = tf.stack(dist_targets_list)
  batch_dist_weights = tf.stack(dist_weights_list)
