...
from utils import model_builder_rtpdd as model_builder
from utils import extra_fields as fields
from utils import tf_example_decoder_rtpdd as tf_example_decoder
from utils.utils_data import tf_preprocess_lidar
...
def freeze_graph_with_def_protos(
  ...
  with tf.Graph().as_default():
    ...
    if optimize_graph:
      ...
      # Use layout_optimizer for tf >=1.5
...
def _lidar_front_tensor_input_placeholder():
  """Returns input placeholder and a 4-D uint8 image tensor."""
  lidar_front_shape = (None, None, None, 2)
  lidar_front_tensor = tf.placeholder(
      dtype=tf.uint8, shape=lidar_front_shape, name='lidar_front_tensor')
  return lidar_front_tensor, lidar_front_tensor

def _lidar_xyz_tensor_input_placeholder():
  """Returns input placeholder and a 4-D uint8 image tensor."""
  lidar_xyz_shape = (None,3)
  lidar_xyz_tensor = tf.placeholder(
      dtype=tf.float32, shape=lidar_xyz_shape, name='lidar_xyz_tensor')
  return lidar_xyz_tensor, lidar_xyz_tensor
...
lidar_placeholder_fn_map = {
    'lidar_front_tensor': _lidar_front_tensor_input_placeholder,
    'lidar_xyz_tensor': _lidar_xyz_tensor_input_placeholder,
}
...
#  label_id_offset = 1
  ...
  dists = postprocessed_tensors.get('detection_dists')
  outputs = {}
  ...
  outputs['detection_dists'] = tf.identity(dists, name='detection_dists')
  ...
  with tf.Graph().as_default():
    with session.Session() as sess:
      ...
      tensor_info_inputs = {
          ...
          'inputs_lidar': tf.saved_model.utils.build_tensor_info(inputs_lidar)}
      tensor_info_outputs = {}
...
def _export_inference_graph(input_type,
  ...
  if lidar_type not in lidar_placeholder_fn_map:
    raise ValueError('Unknown lidar type: {}'.format(lidar_type))
  (placeholder_lidar_tensor,
   lidar_tensor) = lidar_placeholder_fn_map[lidar_type]()
    
  if lidar_type == 'lidar_xyz_tensor':
    im_shape = tf.shape(inputs)
    im_height = im_shape[1]
    im_width = im_shape[2]
    lidar_front = tf_preprocess_lidar(lidar_tensor,dict_calib,im_height,im_width)
    inputs_lidar = tf.to_float(tf.expand_dims(lidar_front,0))
    if init_maxpool:
      inputs_lidar = slim.max_pool2d(inputs_lidar, [3, 3], stride=2)
  else:
    inputs_lidar = tf.to_float(lidar_tensor)
  preprocessed_lidars = detection_model.preprocess(inputs_lidar)
  ...
  _write_saved_model(saved_model_path, frozen_graph_def,
                     placeholder_tensor,placeholder_lidar_tensor,
                     outputs)
...
def export_inference_graph(input_type,
  ...
  _export_inference_graph(input_type, detection_model,
                          ...
                          lidar_type=lidar_type,dict_calib=dict_calib,
